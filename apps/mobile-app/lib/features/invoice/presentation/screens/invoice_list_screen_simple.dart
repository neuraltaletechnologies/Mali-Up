...
class _InvoiceListScreenState extends ConsumerState<InvoiceListScreen> {
  ...
  Widget build(BuildContext context) {
    final invoiceNotifier = ref.watch(invoiceProvider);
    final invoices = invoiceNotifier.invoices;
    final isLoading = invoiceNotifier.isLoading;
    final error = invoiceNotifier.error;

    // Filter invoices based on search and filter
    List<Invoice> filteredInvoices = invoices;
    
    if (_searchController.text.isNotEmpty) {
      filteredInvoices = invoiceNotifier.searchInvoices(_searchController.text);
    }
    
    if (_selectedFilter != 'all') {
      filteredInvoices = invoiceNotifier.getInvoicesByStatus(_selectedFilter);
    }
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: _buildSummaryCard(
                      'Outstanding',
                      invoiceNotifier.getTotalOutstanding(),
                      Colors.orange,
                    ),
                  ),
                ],
