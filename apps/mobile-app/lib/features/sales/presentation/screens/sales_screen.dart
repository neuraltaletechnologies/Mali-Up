...
class _SalesFilterSheetState extends State<_SalesFilterSheet> {
  ...
  @override
  Widget build(BuildContext context) {
    // showAppSheet defaults to a transparent modal background, so this sheet
    // needs its own opaque surface — without it the content floats over
    // whatever is behind the sheet (see _NewSaleSheet/_SaleInfoSheet for the
    // same pattern).
    return Material(
      color: Colors.white,
...
class _NewSaleSheetState extends ConsumerState<_NewSaleSheet> {
  ...
  void initState() {
    ...
    PageTour.maybeAutoStart(
      context: context,
      seenKey: 'page_tour_seen_sales_sheet_v3',
      steps: [
        TourStep(
