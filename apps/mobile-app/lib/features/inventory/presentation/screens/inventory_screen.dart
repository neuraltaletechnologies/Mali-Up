...
ProductType _readType(Map<String, dynamic> item) {
  switch (item['productType'] as String?) {
    ...
    case 'return':
    case 'customerReturn':
      return ProductType.customerReturn;
    case 'manufactured':
...
String _typeName(ProductType t) {
  switch (t) {
    case ProductType.stock:
      return _tr('Stock', 'Stoo');
    case ProductType.perishable:
      return _tr('Perishable', 'Inayoharibika');
    case ProductType.service:
      return _tr('Service', 'Huduma');
    case ProductType.customerReturn:
      return _tr('Return', 'Urejesho');
    case ProductType.manufactured:
      return _tr('Made', 'Ninatengeneza');
  }
}
...
String _healthLabel(int level) {
  switch (level) {
    case 0:
      return _tr('Service', 'Huduma');
    case 3:
      return _tr('Out of Stock', 'Imeisha');
    case 2:
      return _tr('Low Stock', 'Inakwisha');
    default:
      return _tr('Healthy Stock', 'Stoo Ipo');
  }
}
...
String _fullStatusLabel(int level) {
  switch (level) {
    case 5:
      return _tr('Expired', 'Imeisha Muda');
    case 4:
      return _tr('Expiring Soon', 'Karibu Kuisha');
    case 3:
      return _tr('Out of Stock', 'Imeisha');
    case 2:
      return _tr('Low Stock', 'Inakwisha');
...
List<Map<String, dynamic>> _applyFiltersAndSort(
  ...
  final list = src.where((item) {
    ...
    if (typeFilter.isNotEmpty && !typeFilter.contains(_readType(item))) {
      return false;
    }
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // ── Expiry filter ─────────────────────────────────
                      _SheetSectionLabel(
                        _tr('Expiry status', 'Hali ya tarehe'),
                      ),
                      const SizedBox(height: 10),
                      Wrap(
                        spacing: 8,
              ...
              child: Column(
                ...
                children: [
                    ...
                    if (sku.isNotEmpty) ...[
                      ...
                      SizedBox(
                        ...
                        child: OutlinedButton.icon(
                          ...
                          label: Text(
                            _tr('View / Print Barcode', 'Ona / Chapa Nambari'),
                          ),
                  ...
                  if (type == ProductType.manufactured) ...[
                    ...
                    const SizedBox(height: 20),
                    _DetailSectionLabel(
                      _tr('Bill of Materials', 'Orodha ya Malighafi'),
                    ),
                    const SizedBox(height: 14),
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          linkedCount > 0
                              ? _tr(
                                  ...
                                  'Malighafi yatakayokatwa:',
                                )
                              : _tr(
                                  'Ingredients (for reference):',
                                  'Malighafi (kwa kumbukumbu):',
                                ),
                          style: GoogleFonts.dmSans(
                            fontSize: 11,
                            fontWeight: FontWeight.w700,
                        ...
                        ...ingredients.map((i) {
                          ...
                          final hasLink =
                              ((i['matId'] as String?) ?? '').isNotEmpty;
                          final short = _isShort(i);
                          final have =
                              availableStock[(i['matId'] as String?) ?? ''];
                          return Padding(
          ...
          children: [
            ...
            Text(
              _tr(
                ...
                'Malighafi yamekatwa kiotomatiki',
              ),
              textAlign: TextAlign.center,
              style: GoogleFonts.dmSans(
                fontSize: 12,
      ...
      children: [
        ...
        Text(
          ...
          style: GoogleFonts.dmSans(
            ...
            color:
                valueColor ??
                (dim ? AppColors.textMuted : AppColors.navyPrimary),
          ),
        ),
      ],
    );
...
class _BomIngredientEntry {
  ...
  double get cost =>
      double.tryParse(costCtrl.text.replaceAll(RegExp(r'[^0-9.]'), '')) ?? 0;
...
class _ProductFormSheetState extends ConsumerState<_ProductFormSheet> {
  ...
  double get _bomOverheadCost =>
      _bomOverheads.fold(0.0, (s, o) => s + o.amount);
  double get _bomTotalBatchCost => _bomMaterialCost + _bomOverheadCost;
  double get _bomBatchYield =>
      (double.tryParse(_batchYieldCtrl.text) ?? 0).clamp(0.01, double.infinity);
  double get _bomCostPerUnit => _bomTotalBatchCost / _bomBatchYield;

  double get _buyVal {
    if (_type == ProductType.manufactured) return _bomCostPerUnit;
    return double.tryParse(_buyCtrl.text.replaceAll(RegExp(r'[^0-9.]'), '')) ??
        0;
  }
  ...
  void initState() {
    ...
    if (item != null) {
      ...
      final stk =
          (item['currentStock'] as num?)?.toDouble() ?? _stock(item).toDouble();
      _stockCtrl.text = stk % 1 == 0 ? stk.toStringAsFixed(0) : stk.toString();
      final r = parseStock(item['reorderPoint']);
      ...
      if (_type == ProductType.manufactured) {
        ...
        if (rawBomIng is List) {
          for (final ing in rawBomIng.whereType<Map<String, dynamic>>()) {
            ...
            e.qtyCtrl.text = ((ing['qty'] as num?)?.toDouble() ?? 1)
                .toStringAsFixed(
                  ((ing['qty'] as num?)?.toDouble() ?? 1) % 1 == 0 ? 0 : 2,
                );
            e.costCtrl.text = ((ing['costPer'] as num?)?.toDouble() ?? 0)
                .toStringAsFixed(0);
        ...
        if (rawBomOv is List) {
          for (final ov in rawBomOv.whereType<Map<String, dynamic>>()) {
            ...
            e.amountCtrl.text = ((ov['amount'] as num?)?.toDouble() ?? 0)
                .toStringAsFixed(0);
            _bomOverheads.add(e);
          }
        }
        final yield_ = (item['bomBatchYield'] as num?)?.toDouble() ?? 1;
        _batchYieldCtrl.text = yield_ % 1 == 0
  ...
  Future<void> _save() async {
    ...
    if (isManufactured) {
      if (_bomIngredients.isEmpty) {
        _snack(
          _tr('Add at least one ingredient', 'Ongeza kiungo kimoja angalau'),
        );
        return;
      }
      for (final i in _bomIngredients) {
      ...
      if (_bomBatchYield <= 0) {
        _snack(
          _tr(
            'Enter a batch yield greater than 0',
            'Ingiza matokeo ya kundi zaidi ya 0',
          ),
        );
        return;
      }
    ...
    for (final u
      ...
      final p =
          double.tryParse(
            u.priceCtrl.text.replaceAll(RegExp(r'[^0-9.]'), ''),
          ) ??
          0;
      if (p <= 0) {
        _snack(
          _tr(
            'Enter a price for each selling unit',
            'Ingiza bei kwa kila kitengo',
          ),
        );
        return;
      }
                      ...
                      children: [
                        Expanded(
                          child: Text(
                            _restockTarget != null
                                ? _tr(
                                    'Restock: ${_restockTarget!.name}',
                                    'Ongeza Stoo: ${_restockTarget!.name}',
                                  )
                                : _isEdit
                                ? _tr('Edit Product', 'Hariri Bidhaa')
                                : _type == ProductType.customerReturn
                  ...
                  child: Column(
                    ...
                    children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              Padding(
                                ...
                                child: Text(
                                  _tr(
                                    'Already in your inventory — restock?',
                              ...
                              ...nameSuggestions.asMap().entries.map((e) {
                                ...
                                return InkWell(
                                  ...
                                  borderRadius: BorderRadius.vertical(
                                    bottom: idx == nameSuggestions.length - 1
                      ...
                      if (!isReturn && config.showBatchNumber) ...[
                        _FormLabel(
                          '${_tr('Batch Number', 'Nambari ya Kundi')} (${_tr("optional", "hiari")})',
                        ),
                        const SizedBox(height: 6),
                        _FormField(
                          ctrl: _batchCtrl,
                      ...
                      if (!isReturn && config.showBrand) ...[
                        _FormLabel(
                          '${_tr('Brand', 'Chapa')} (${_tr("optional", "hiari")})',
                        ),
                        const SizedBox(height: 6),
                        _FormField(
                          ctrl: _brandCtrl,
                      ...
                      if (!isReturn && config.showWarrantyPeriod) ...[
                        _FormLabel(
                          '${_tr('Warranty Period', 'Kipindi cha Dhamana')} (${_tr("optional", "hiari")})',
                        ),
                        const SizedBox(height: 6),
                        _FormField(
                          ctrl: _warrantyCtrl,
                      ...
                      if (isManufactured) ...[
                        ...
                        _AddRowButton(
                          ...
                          onTap: () => setState(
                            () => _bomIngredients.add(_BomIngredientEntry()),
                          ),
                        ),
                        ...
                        _AddRowButton(
                          ...
                          onTap: () => setState(
                            () => _bomOverheads.add(_BomOverheadEntry()),
                          ),
                        ),
                                ...
                                children: [
                                  ...
                                  Text(
                                    _tr(
                                      ...
                                      'Vipande vinavyotengenezwa kwa kundi',
                                    ),
                                    style: GoogleFonts.dmSans(
                                      fontSize: 10,
                                      color: AppColors.textMuted,
                                ...
                                children: [
                                  ...
                                  Text(
                                    _tr(
                                      ...
                                      'Kitengo cha bidhaa iliyotengenezwa',
                                    ),
                                    style: GoogleFonts.dmSans(
                                      fontSize: 10,
                                      color: AppColors.textMuted,
                      ...
                      if (!isReturn && _type != ProductType.service) ...[
                          ...
                          children: [
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  _FormSectionLabel(
                                    _tr('Selling Units', 'Vitengo vya Mauzo'),
                                  ),
                                  const SizedBox(height: 2),
                                  Text(
                                    _tr(
                        ...
                        GestureDetector(
                          onTap: () =>
                              setState(() => _sellingUnits.add(_UnitEntry())),
                          child: Container(
                            width: double.infinity,
                      ...
                      if (isReturn) ...[
                        ...
                        const SizedBox(height: 20),
                        _FormSectionLabel(
                          _tr('Quantity Returned', 'Kiasi Kilichorudishwa'),
                        ),
                        const SizedBox(height: 12),
                        _FormField(
                          ctrl: _stockCtrl,
...
class _BomIngredientCardState extends State<_BomIngredientCard> {
  ...
  void _applyInventoryItem(InventoryItem item) {
    setState(() {
      ...
      if (item.costPrice > 0) {
        widget.entry.costCtrl.text = item.costPrice.toStringAsFixed(0);
      }
      ...
      child: Column(
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              hintText: _tr(
                'e.g. Flour, Cement, Milk',
                'k.m. Unga, Saruji, Maziwa',
              ),
              hintStyle: GoogleFonts.dmSans(
                fontSize: 13,
                color: AppColors.textDisabled,
                ...
                children: suggestions.asMap().entries.map((e) {
                  ...
                  return InkWell(
                    ...
                    borderRadius: BorderRadius.vertical(
                      ...
                      bottom: idx == suggestions.length - 1
...
class _ExpiryDateButton extends StatelessWidget {
  ...
  bool get _isExpired => date != null && date!.isBefore(DateTime.now());
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: _tr(
                      ...
                      'Tafuta mteja au nambari ya ankara',
                    ),
                    hintStyle: GoogleFonts.dmSans(
                      fontSize: 13,
                      color: AppColors.textDisabled,
