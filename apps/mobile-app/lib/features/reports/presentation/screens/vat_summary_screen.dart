    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // Output VAT (Sales)
          _VatSection(
            title: _tr('Output VAT (on Sales)', 'VAT ya Uzalishaji (kwa Mauzo)'),
            color: AppColors.success,
            icon: Icons.arrow_upward_rounded,
            children: [
              _VatRow(label: _tr('Taxable Sales Amount', 'Kiasi cha Mauzo Yanayotozwa'), value: formatCurrency(report.taxableSalesAmount)),
              _VatRow(label: _tr('Number of Taxable Invoices', 'Idadi ya Ankara Zinazotosha'), value: '${report.taxableSalesCount}', isAmount: false),
              _VatRow(label: _tr('VAT Collected (Output)', 'VAT Iliyokusanywa (Output)'), value: formatCurrency(report.vatCollectedOnSales), bold: true),
            ],
          ),
          ...
          // Input VAT (Purchases)
          _VatSection(
            title: _tr('Input VAT (on Purchases)', 'VAT ya Mauzo ya Ndani (kwa Manunuzi)'),
            color: AppColors.warning,
            icon: Icons.arrow_downward_rounded,
            children: [
              _VatRow(label: _tr('Taxable Purchases Amount', 'Kiasi cha Manunuzi Yanayotozwa'), value: formatCurrency(report.taxablePurchasesAmount)),
              _VatRow(label: _tr('Taxable Purchase Records', 'Rekodi za Manunuzi Yanayotosha'), value: '${report.taxablePurchasesCount}', isAmount: false),
              _VatRow(label: _tr('VAT Paid (Input)', 'VAT Iliyolipwa (Input)'), value: formatCurrency(report.vatPaidOnPurchases), bold: true),
            ],
          ),
          const SizedBox(height: 16),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: isPayable ? AppColors.error : AppColors.success, width: 1.5),
            ),
            child: Column(
              children: [
                Text(
          ...
          ReportExportRow(
            onPdf: () => ReportExportService.shareVatPdf(report, periodLabel),
            onCsv: () async {
...
class _VatRow extends StatelessWidget {
  ...
  const _VatRow({required this.label, required this.value, this.bold = false, this.isAmount = true});
