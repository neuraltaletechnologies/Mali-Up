...
abstract final class ReportExportService {
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          pw.SizedBox(height: 4),
          ...report.expenseByCategory.entries.map(
            (e) => _summaryRow(e.key, _fmt(e.value)),
          ),
          pw.Divider(),
  ...
  static String pnlCsv(PnlReport report, String period) {
    final rows = [
      ...
      _csvRow(['EXPENSES', '']),
      ...report.expenseByCategory.entries.map(
        (e) => _csvRow([e.key, e.value.toStringAsFixed(2)]),
      ),
      ...
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          _summaryRow(
            ...
            _fmt(report.averageInvoiceValue),
          ),
          pw.SizedBox(height: 12),
          pw.Text(
            'By Payment Method',
  ...
  static String salesCsv(SalesReport report, String period) {
    final rows = [
      ...
      _csvRow([
        'Avg Invoice Value',
        report.averageInvoiceValue.toStringAsFixed(2),
      ]),
      _csvRow(['', '']),
      _csvRow(['BY PAYMENT METHOD', '']),
      _csvRow(['Method', 'Amount']),
      ...report.byPaymentMethod.entries.map(
        (e) => _csvRow([e.key, e.value.toStringAsFixed(2)]),
      ),
      _csvRow(['', '']),
      _csvRow(['BY PRODUCT', '']),
  ...
  static Future<void> shareExpensePdf(
        ...
        build: (ctx) => [
          ...
          ...report.byCategory.entries.toList().asMap().entries.map((e) {
            final pct = report.totalExpenses > 0
                ? (e.value.value / report.totalExpenses * 100).toStringAsFixed(
                    1,
                  )
                : '0.0';
            return _tableRow([
      ...
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          _summaryRow(
            ...
            _fmt(report.vatCollectedOnSales),
          ),
  ...
  static String vatCsv(VatSummary report, String period) {
    return [
      ...
      _csvRow([
        ...
        report.taxableSalesAmount.toStringAsFixed(2),
      ]),
      ...
      _csvRow([
        'Taxable Purchases Amount',
        report.taxablePurchasesAmount.toStringAsFixed(2),
      ]),
  ...
  static Future<void> shareArAgingPdf(
    ArAgingReport report,
    String period,
  ) async {
    final doc = pw.Document();
    doc.addPage(
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          for (final bucket in [
            ...
            ...bucket.items.asMap().entries.map(
              (e) => _tableRow([
                (e.value['customerName'] ?? e.value['customer'] ?? 'Unknown')
                    .toString(),
  ...
  static String arAgingCsv(ArAgingReport report) {
    ...
    rows.add(
      _csvRow(['', '', '', report.grandTotal.toStringAsFixed(2), 'TOTAL']),
    );
    return rows.join('\n');
  }
  ...
  static Future<void> shareApAgingPdf(
    ApAgingReport report,
    String period,
  ) async {
    final doc = pw.Document();
    doc.addPage(
  ...
  static String apAgingCsv(ApAgingReport report) {
    final rows = [
      _csvRow(['Vendor', 'Category', 'Age (days)', 'Amount', 'Bucket']),
    ];
    for (final bucket in [
      report.current,
      report.days31to60,
  ...
  static Future<void> shareCashFlowPdf(
      ...
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          _summaryRow(
            ...
            '(${_fmt(report.operatingOutflows)})',
          ),
          _summaryRow(
            'Net Operating Cash Flow',
            _fmt(report.netOperating),
            bold: true,
          ),
          pw.Divider(thickness: 2),
          _summaryRow(
            'Closing Balance',
            _fmt(report.closingBalance),
            bold: true,
          ),
        ],
      ),
  ...
  static Future<void> shareBalanceSheetPdf(
    BalanceSheet report,
    String date,
  ) async {
    final doc = pw.Document();
  ...
  static String balanceSheetCsv(BalanceSheet report) {
    return [
      ...
      _csvRow([
        'Cash & Equivalents',
        report.cashAndEquivalents.toStringAsFixed(2),
      ]),
      _csvRow([
        'Accounts Receivable',
        report.accountsReceivable.toStringAsFixed(2),
      ]),
  ...
  static Future<void> shareInventoryPdf(
