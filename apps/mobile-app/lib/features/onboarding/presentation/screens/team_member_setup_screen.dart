...
class _TeamMemberSetupScreenState extends ConsumerState<TeamMemberSetupScreen>
  ...
  Future<void> _advanceToConfirm() async {
    ...
    // offline user isn't asked to fill in email + PIN twice across two
    // steps only to be told at the very end that saving needs internet.
...
class _InvitationBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text(
          sw ? 'Umepata mwaliko 🎉' : 'Your got invited 🎉',
          style: GoogleFonts.dmSans(
            fontSize: 26,
