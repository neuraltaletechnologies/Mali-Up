...
export default function PlansPage() {
  const { data, loading, error, refetch } = useAdminFetch(useCallback(() => fetchPlans(), []))
  ...
  if (loading) {
    return (
      <div>
        <PageHeader title="Subscription Plans" description="Loading…" />
  ...
  if (error || !localPlans) {
    return (
      <div>
        <PageHeader title="Subscription Plans" description="Failed to load" />
